...
def _signup_and_login(client, email: str) -> tuple[str, str]:
    ...
    # require_paid_or_read_only now blocks write actions (sending a
    # message) for a TRIALING account, and this file's tests are about
    # messaging mechanics, not trial-gating, so upgrade to a real paid
    # plan here rather than adding this to every individual test.
    client.put(
