...
def test_notification_delivery_is_recorded_and_listable(client, db_session, monkeypatch):
    ...
    # Four deliveries now: signup itself sends auth.account_activated,
    # org.organization_created, and auth.verify_email_address, plus the
    # number.activated sent explicitly above - the account's notification
    # list is account-wide, not filtered to one event.
...
def test_mark_all_notifications_read(client, db_session, monkeypatch):
    ...
    # 5, not 2 - signup itself sends auth.account_activated,
    # org.organization_created, and auth.verify_email_address, in addition
    # to the two number.activated notifications sent explicitly above.
    response = client.post("/notifications/read-all", headers={"Authorization": f"Bearer {token}"})
    assert response.status_code == 200
    assert response.json()["marked_read"] == 5
