...
def test_incoming_call_rings_all_configured_destinations_simultaneously(client, db_session):
    ...
    # Bare "<Number>", no attributes - build_ring_group_response no longer
    # attaches a per-leg statusCallback (real bug fix: that callback fires
    # with the CHILD leg's CallSid, which never matches any CallRecord, so
    # it never captured anything real - see that function's own docstring).
