...
def test_owner_can_add_a_viewer_team_member(client):
    owner_token = _signup_and_login(client, "viewerowner1@example.com")
    _add_viewer(client, {"Authorization": f"Bearer {owner_token}"}, "viewer1@example.com")


def test_viewer_can_list_numbers(client, monkeypatch):
    ...
    viewer_token = _add_viewer(client, owner_headers, "viewer2@example.com")
...
def test_viewer_cannot_list_team_members(client):
    ...
    owner_headers = {"Authorization": f"Bearer {owner_token}"}
    viewer_token = _add_viewer(client, owner_headers, "viewer3@example.com")

    response = client.get("/team/members", headers={"Authorization": f"Bearer {viewer_token}"})
    assert response.status_code == 403


def test_viewer_cannot_suspend_a_number(client, monkeypatch, caplog):
    ...
    viewer_token = _add_viewer(client, owner_headers, "viewer4@example.com")
...
def test_viewer_cannot_reserve_a_number(client):
    owner_token = _signup_and_login(client, "viewerowner5@example.com")
    owner_headers = {"Authorization": f"Bearer {owner_token}"}
    viewer_token = _add_viewer(client, owner_headers, "viewer5@example.com")
...
def test_viewer_cannot_add_team_members(client):
    owner_token = _signup_and_login(client, "viewerowner6@example.com")
    owner_headers = {"Authorization": f"Bearer {owner_token}"}
    viewer_token = _add_viewer(client, owner_headers, "viewer6@example.com")
...
def test_viewer_cannot_mark_notification_read(client):
    owner_token = _signup_and_login(client, "viewerowner7@example.com")
    owner_headers = {"Authorization": f"Bearer {owner_token}"}
    viewer_token = _add_viewer(client, owner_headers, "viewer7@example.com")
...
def test_viewer_can_still_set_up_their_own_mfa(client):
    ...
    owner_headers = {"Authorization": f"Bearer {owner_token}"}
    viewer_token = _add_viewer(client, owner_headers, "viewer8@example.com")
...
def test_viewer_can_still_update_their_own_phone_number(client):
    owner_token = _signup_and_login(client, "viewerowner9@example.com")
    owner_headers = {"Authorization": f"Bearer {owner_token}"}
    viewer_token = _add_viewer(client, owner_headers, "viewer9@example.com")
