...
def upgrade() -> None:
    ...
    #
    # Guarded with a NOT EXISTS check (found unguarded during a later merge
    # - see d05ac876b3ab's docstring, which independently seeds this exact
    # same (capability, role) row and already anticipated this collision):
    # a merged chain that runs both migrations against one DB hits
    # uq_staff_capability_grant without the guard.
    op.execute(
        sa.text(
            """
