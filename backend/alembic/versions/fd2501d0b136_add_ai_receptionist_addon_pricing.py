...
def upgrade() -> None:
    ...
    op.add_column(
        "plans",
        sa.Column("ai_receptionist_minutes_included", sa.Integer(), nullable=False, server_default="0"),
    )
    op.execute("UPDATE plans SET ai_receptionist_minutes_included = 50 WHERE plan_code = 'pro'")
    op.execute("UPDATE plans SET ai_receptionist_minutes_included = 150 WHERE plan_code = 'scale'")
...
def downgrade() -> None:
    op.drop_column("plans", "ai_receptionist_minutes_included")
