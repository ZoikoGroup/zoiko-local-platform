...
export default function CallsPage() {
  ...
  async function handleBridgeCall() {
    if (!token || !fromNumber || !toNumber) return;
    setCallBusy(true);
    setCallError(null);
    setCallSuccess(null);
    try {
      const customerNumber = toNumber;
      await placeBridgeCall(token, { to: toNumber, from: fromNumber });
      setCallSuccess(
        `Calling your forwarding number now — answer it to be connected live to ${customerNumber}.`
      );
      setToNumber("");
      await loadAll();
    } catch (err) {
          ...
          <form onSubmit={handlePlaceCall} className="space-y-3">
            <div className="flex flex-wrap items-end gap-3">
              ...
              <button
                ...
                disabled={callBusy || !fromNumber || !toNumber}
                className="bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 text-white text-sm font-medium rounded-lg px-4 py-2"
                title="Rings your forwarding number first, then connects you live to this number once you answer"
              >
                {callBusy ? "Calling..." : "Talk Live"}
              </button>
            </div>
            <p className="text-xs text-slate-500">
              <strong>Announce</strong> plays a one-way message and hangs up. <strong>Talk Live</strong> rings your own configured forwarding number first, then connects you to a real, live, two-way conversation with the person you're calling once you pick up.
            </p>
            <div>
